...
#TODO: Implement FRU on cells.py
...
class FRUCell(tf.contrib.rnn.RNNCell):
    ...
    Use with DRNN because of time dependency
    """

    def __init__(self, num_stats, freqs, freqs_mask, output_dims, recur_dims,
        seq_len, summarize=True, linear_out=False,include_input=False,
        activation=tf.nn.relu, state_is_tuple=True):
        """
         Initialize FRU Cell

        Args:
            num_stats: Number .
            freqs: List of frequencies to consider
            freqs_mask: Frequency to pad freqs[i] when freqs[i]==0
            output_dims: FRU output dimension
            recur_dims: Recurrent vector dimension
            seq_len: Length of the sequence
            summarize: --
            linear_out: if False then do activation on output
            activation: Activation function on output
            state_is_tuple: --
        """
    ...
    def __call__(self, inputs, state_tuple, scope=None):
        ...
        Args:
            ...
            state_tuple: state time tuple i.e (Nxdxh,1)

        Returns:
            output: N x d x output_dims tensor of parameters for the coditional density
                of each dimension.
            out_state: function that takes in a batch size and base component
                distribution and outputs a tensor of batch_size x d of samples.
        """

        state, cur_time_step = state_tuple
        cur_time_step = tf.cast(cur_time_step,tf.float32)

        with tf.variable_scope(scope or type(self).__name__):
            self._freqs = tf.reshape(tf.get_variable("frequency",
                initializer=self._freqs_array, trainable=False), [1, -1, 1])
            self._phases = tf.reshape(tf.get_variable("phase", [self._nfreqs],
                initializer=tf.truncated_normal_initializer(stddev=0.1,
                    dtype=tf.float32), trainable=True), [1, -1, 1])
            self._freqs_mask = tf.reshape(tf.get_variable("frequency_mask",
                initializer=self._freqs_mask_array, trainable=False),
                    [1, -1, 1])
            ...
            with tf.variable_scope('out_state'):
                ...
                out_state = tf.reshape(self._freqs_mask*state_tensor +
                                       1.0/self._seq_len*tf.cos(2.0*math.pi/\
                                       self._seq_len*tf.reshape(cur_time_step, \
                                       shape=[-1, 1, 1])*self._freqs + \
                                       2.0*math.pi*self._phases)*stats_tensor,
